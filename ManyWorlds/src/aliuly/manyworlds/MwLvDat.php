...
class MwLvDat extends BasicCli{
	...
	public function __construct($owner){
		...
		$this->enableSCmd("lvdat", [
			"usage" => mc::_("<world> [attr=value]"),
			"help" => mc::_("Change level.dat values"),
			"permission" => "mw.cmd.lvdat",
			"aliases" => ["lv"],
		]);
		$this->enableSCmd("fixname", [
			"usage" => mc::_("<world>"),
	...
	public function onSCommand(CommandSender $c, Command $cc, $scmd, $data, array $args){
		...
		foreach($args as $kv){
			...
			switch(strtolower($k)){
				case "spawn":
					...
					if(($x = intval($x)) == $cpos->getX() && ($y = intval($y)) == $cpos->getY() && ($z = intval($z)) == $cpos->getZ()){
		...
		if($changed){
			...
			if($unload){
				$c->sendMessage(TextFormat::RED . mc::_("CHANGES WILL NOT TAKE EFFECT UNTIL UNLOAD"));
			}
		}else{
