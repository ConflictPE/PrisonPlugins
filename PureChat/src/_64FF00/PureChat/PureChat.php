...
class PureChat extends PluginBase{
	...
	*/
	...
	public function applyPCTags($string, Player $player, $message, $levelName){
		...
		if($this->nicks instanceof Plugin){
			$string = str_replace("{nick}", $this->nicks->getNick($player), $string);
		}else{
			$string = str_replace("{nick}", "", $string);
		}
	...
	*/
	...
	public function getPrefix(Player $player, $levelName = null){
		if($levelName === null){
			...
			if(!isset($worldData["prefix"]) || $worldData["prefix"] === null) return "";
	...
	public function getSuffix(Player $player, $levelName = null){
		if($levelName === null){
			...
			if(!isset($worldData["suffix"]) || $worldData["suffix"] === null) return "";
	...
	private function fixOldConfig(){
		...
		if(!isset($tempData["default-factions-plugin"])) $tempData["default-factions-plugin"] = null;
	...
	private function loadNicksPlugin(){
		$nicks = $this->getServer()->getPluginManager()->getPlugin("Nicks");
		if($nicks instanceof Plugin) $this->nicks = $nicks;
	}
}
