...
class EventListener implements Listener {
	...
	public function onBreak(BlockBreakEvent $event) {
		foreach($event->getDrops() as $drop) {
			$item = Item::get(...$drop);
			$event->getPlayer()->getInventory()->addItem($item); // Add the item to the players inventory (server side)
			$event->getPlayer()->getInventory()->sendContents($event->getPlayer()); // Send the updated contents to the player (client sided)
		}
		$event->setDrops([0, 0, 0]);
	}
