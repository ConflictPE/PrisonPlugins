	...
	public function execute(CommandSender $sender, $label, array $args) {
		...
		if($this->plugin->getEconomy()->getMoney($sender) >= $this->plugin->getGroupManager()->getPrice($rank)) {
			if($this->plugin->rankUp($sender)) {
				...
				if($this->plugin->getConfig()->get('broadcastMessageOnRankUp'))
					$this->plugin->getServer()->broadcastMessage($this->plugin->messages->getMessage('pc.command.rankup.rankedUpBroadcast', $sender->getName(), $rank->getName()));
				return \true;
			} else {
