...
class WpList extends BaseWp{
	...
	private function wpDetails(CommandSender $c, $world, $pageNumber){
		...
		if(isset($wcfg["protect"])) $txt[] = TextFormat::AQUA . mc::_("Protect:  ") . TextFormat::WHITE . $wcfg["protect"];

		if(isset($wcfg["max-players"])) $txt[] = TextFormat::AQUA . mc::_("Max Players:  ") . TextFormat::WHITE . $wcfg["max-players"];
		if(isset($wcfg["gamemode"])) $txt[] = TextFormat::AQUA . mc::_("Gamemode:  ") . TextFormat::WHITE . MPMU::gamemodeStr($wcfg["gamemode"]);
		...
		if(isset($wcfg["border"])) $txt[] = TextFormat::AQUA . mc::_("Border: ") . TextFormat::WHITE . implode(",", $wcfg["border"]);
		if(isset($wcfg["auth"])) $txt[] = TextFormat::AQUA . mc::_("Auth List(%1%): ", count($wcfg["auth"])) . TextFormat::WHITE . implode(",", $wcfg["auth"]);

		if(isset($wcfg["unbreakable"])) $txt[] = TextFormat::AQUA . mc::_("Unbreakable(%1%): ", count($wcfg["unbreakable"])) . TextFormat::WHITE . implode(",", $wcfg["unbreakable"]);
		if(isset($wcfg["bancmds"])) $txt[] = TextFormat::AQUA . mc::_("Ban Commands(%1%): ", count($wcfg["bancmds"])) . TextFormat::WHITE . implode(",", $wcfg["bancmds"]);
		if(isset($wcfg["banitem"])) $txt[] = TextFormat::AQUA . mc::_("Banned(%1%): ", count($wcfg["banitem"])) . TextFormat::WHITE . implode(",", $wcfg["banitem"]);
	...
	private function attrList($wcfg){
		...
		if(isset($wcfg["no-explode"])) $attr[] = mc::_("notnt:") . $wcfg["no-explode"];
		if(isset($wcfg["border"])) $attr[] = mc::_("border");
		if(isset($wcfg["auth"])) $attr[] = mc::_("auth(%1%)", count($wcfg["auth"]));
		if(isset($wcfg["max-players"])) $attr[] = mc::_("max:") . $wcfg["max-players"];
		if(isset($wcfg["gamemode"])) $attr[] = mc::_("gm:") . $wcfg["gamemode"];
		if(isset($wcfg["unbreakable"])) $attr[] = mc::_("ubab:") . count($wcfg["unbreakable"]);
		if(isset($wcfg["bancmds"])) $attr[] = mc::_("bc:") . count($wcfg["bancmds"]);
		if(isset($wcfg["banitem"])) $attr[] = mc::_("bi:") . count($wcfg["banitem"]);
		return $attr;
	}
